...
def fastHough(f, sobel_threshold, hough_threshold):
    if len(f.shape) == 3:
        return {
    ...
    n = f.shape[0]; m = f.shape[1]
    y_end = f.shape[0] - 1; x_end = f.shape[1] - 1
    ...
    for y in range(0, n):
        for x in range(0, m):
            ...
            f_x = int(f[yMinus1][xPlus1]) - int(f[yMinus1][xMinus1]) + 2 * int(f[y][xPlus1]) \
                - 2 * int(f[y][xMinus1]) + int(f[yPlus1][xPlus1]) - int(f[yPlus1][xMinus1])
            # computing f_y
            f_y = int(f[yPlus1][xMinus1]) - int(f[yMinus1][xMinus1]) + 2 * int(f[yPlus1][x]) \
                - 2 * int(f[yMinus1][x]) + int(f[yPlus1][xPlus1]) - int(f[yMinus1][xPlus1])
            ...
            if (gradient >= sobel_threshold):
                ...
                r = np.round(x * np.cos(theta) + (y_end - y) * np.sin(theta)).astype(int)
                # incrementing Hough matrix
                theta_deg = np.round(np.degrees(theta)).astype(int)
                hough[diag - r, theta_deg + 90] = hough[diag - r, theta_deg + 90] + 1
    ...
    # iterating through the Hough matrix
    print('Drawing contour lines...')
    for ln in range(0, 2 * diag):
        for col in range(0, 181):
            confidence = hough[ln][col].astype(np.ubyte)
            if (confidence > hough_threshold):  # if line defined by (r, theta) exceeds confidence threshold,
                theta = np.radians(col - 90)    # draw lines by converting (r, theta) to (y, x) points
                r = diag - ln                   
                cosine = np.cos(theta)          
                sine = np.sin(theta)            
                for x in range(0, m):           
